...
module.exports = ({ database, config }) => ({
  ...
  onboard: (req, res) => {
    return database.accounts.getAccount(req.params.account_id)
      .then(account => {
        ...
        const command = `
          ...
          container_port=$(
            ...
            ssh-add ${process.env['UGLY_PRIVKEY_PATH']} >/dev/null
        ...
        `
        const containerPort = execSync(command).toString()
        ...
        return database.machines.createMachine(machine)
          .then(() => {
            res.json({
              success: true,
              message: 'Account onboarded succesfully',
              machine
            })
          })
      })
